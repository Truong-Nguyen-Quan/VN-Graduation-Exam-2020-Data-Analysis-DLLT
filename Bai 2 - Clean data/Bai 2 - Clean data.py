...
with open('clean data_csv.txt', encoding="utf8", mode="w", newline="") as file:
...
for data in datas:
	...
	dd = str(int(birth_date[0]))
	mm = str(int(birth_date[1]))
	yy = str(int(birth_date[2]))
	data = [sbd_str,name.title(),dd,mm,yy]

	#process scores
	subject_all = ["toán", "ngữ văn", "khxh", "khtn", "lịch sử", "địa lí", "gdcd", "sinh học", "vật lí", "hóa học", "tiếng anh"]
	...
	with open('clean data_csv.txt', encoding="utf8", mode="a", newline="") as file_csv:
